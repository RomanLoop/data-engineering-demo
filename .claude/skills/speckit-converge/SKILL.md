...
- Entities and their Acceptance Scenarios
...
- **Requirements inventory**: one stable key per FR-### / SC-### / entity acceptance
  scenario (e.g. `E1/AC2`), plus the plan decisions and constitution principles that
...
- **CRITICAL**: violates a constitution MUST principle, or a `missing`/`contradicts` gap
  that blocks baseline functionality of a P1 entity.
...
3. Emit one checklist item per actionable finding, ordered CRITICAL/HIGH first, assigning
   ...
   `E1/AC2`, `plan: storage decision`, `Constitution II`.
