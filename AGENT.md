...
| 12 | Partitioning `orders` / `orderrows` (2026-09-10) | **`orders` partitioned by `order_year` (= `year(OrderDate)`, int) at bronze + silver + gold**; `serving` inherits it. **`orderrows` is NOT partitioned** — it carries no date column, and the only way to give it `order_year` is a cross-entity join to `orders`, which silver forbids (§5) and which isn't worth a documented exception for a demo. `order_year` is kept **last** in every contract (Delta places the partition column last physically). `order_year` is in the silver SCD1 `merge_update_columns` but **not** in `_scd1_hash` (it's a pure function of `order_date`, already hashed). **Partition pruning of the SCD1 MERGE**: a *static* `incremental_predicates=['DBT_INTERNAL_DEST.order_year = DBT_INTERNAL_SOURCE.order_year']` (the standard Delta idiom — engine prunes target files to the source's partitions). It **must** be static: dbt's runtime `config()` is a no-op (`RuntimeConfigObject.__call__` returns `""`), so a `run_query`-computed year list can't be injected, and Fabric Delta MERGE rejects any subquery in `ON` (`[DELTA_UNSUPPORTED_SUBQUERY]`) — both tried and rejected 2026-09-10. Caveat: correct only while an update never changes a row's `order_year`; the delta simulator only mutates `DeliveryDate`/`CurrencyCode`. A real cross-year `order_date` correction → duplicate `order_key`, caught loudly by the `unique` test; fix with `--full-refresh`. **Bronze rollout is a one-time manual `DROP TABLE` + plain rebuild** (append-only ⇒ `--full-refresh` forbidden; a partition column can't be added by `ALTER`) — done 2026-09-10, row counts unchanged (serving/silver/bronze = 980,966; 11 year-partitions 2016–2026). |
| 13 | Dagster orchestration (`specs/003-dagster-orchestration`, 2026-09-11) | Roadmap item 7 (§10) — all three capabilities implemented and live-validated against Fabric; one known capacity limitation remains (below), so not marked fully done. **`dbt-core` downgraded 1.12.3 → 1.11.15** — every published `dagster-dbt` caps `dbt-core<1.12`; `dbt-fabricspark` only needs `>=1.8.0`; re-validated clean (`dbt parse`, full `sqlfluff lint`, and live `dbt build`s through Dagster). **dbt-as-Dagster-assets uses classic `@dbt_assets`, not `dagster_dbt.DbtProjectComponent`** — the Component reliably hit a Windows `PermissionError` rebuilding its `.local_defs_state` cache on every reload after the first (reproduced 3+ times; likely this repo's synced-folder location colliding with the Component's delete-then-recreate caching); a separate `LongPathsEnabled=0` issue was found and fixed along the way but didn't resolve the permission error. `@dbt_assets` has no state-staging directory and is stable. **Capability 2** (`landing_assets.py`, one asset per entity: OneLake upload + landing bootstrap) and **Capability 3** (`schedules.py`, `delta_simulation` asset + `delta_load_job` + `delta_load_schedule`, stopped by default) are both implemented, wired via `meta.dagster.asset_key` on the dbt sources, and live-validated at the individual/vertical-slice level: single-model materialize, single landing-asset materialize (real OneLake upload + bootstrap), landing→bronze materializing together with correct dedup, and an immediate no-op rerun proving idempotency — all `RUN_SUCCESS` against live Fabric. Two real bugs found only by live testing and fixed: `contoso_currencyexchange.py`'s simulator has a different CLI (`--new-date`, not `--new`) that argparse's prefix-matching silently mis-resolved; and `DbtCliResource.cli(..., context=...)` is only valid inside a `@dbt_assets`-generated asset, not a plain `@asset` like the landing ones. **Known limitation**: materializing the *entire* 41-asset graph in one Dagster run (8+ sequential `dbt` CLI subprocess invocations) reliably hits `TooManyRequestsForCapacity` (HTTP 430) — **correction (2026-09-11): the capacity was actually F8 at test time, not F32** (it's scaled down to F8 overnight for cost reasons and back up to F32 during active work — see below); F8 gives only 16 base / 48 burst Spark VCores and a queue limit of 8 (vs. F32's 64/192/32), and Livy/interactive sessions aren't queued at all when capacity is full — they're rejected immediately (HTTP 430), unlike pipeline/scheduler-triggered jobs. `reuse_session: true` also doesn't reliably hold across that many rapid sequential invocations regardless of SKU (matches decision 8's caveat) — several `InProgress` Livy sessions were repeatedly found open concurrently via the Fabric jobs-instances API, consistent with each `dbt` subprocess sometimes opening its own session rather than reusing one. Recovered via the documented Livy-session-cancel pattern each time; for real use, prefer scoped/incremental materializations (by entity or layer) over one all-at-once run, especially outside active-hours F32 windows. Full detail: `specs/003-dagster-orchestration/research.md` Decisions 8/10, `tasks.md` T014/T025-028/T031-033. |
| 14 | Fabric capacity is scaled down overnight (2026-09-11) | The Fabric capacity backing this project is **F32 during active work hours** and gets **scaled down to F8 overnight for cost savings**, then back up to F32. Decision 13's `TooManyRequestsForCapacity` errors happened during an F8 window. Relevant for any future Dagster schedule/automation: a run (including `delta_load_schedule`, stopped by default per decision 13) landing during an overnight F8 window has roughly 4× less headroom (16 base / 48 burst VCores / queue limit 8, vs. F32's 64/192/32) and a materially higher chance of hitting 430s — worth checking current capacity SKU before enabling any schedule or running a large batch. |
