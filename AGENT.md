...
| 13 | Dagster orchestration (`specs/003-dagster-orchestration`, 2026-09-11) | Roadmap item 7 (§10), in progress. **`dbt-core` downgraded 1.12.3 → 1.11.15** — every published `dagster-dbt` (through the latest `0.29.21`) caps `dbt-core<1.12`; `dbt-fabricspark` only needs `>=1.8.0`. Re-validated: `dbt parse`/`sqlfluff lint` clean project-wide at 1.11.15 (this pass, run while Fabric was unreachable, incidentally exercised the P2 anti-join's non-incremental branch for the first time and caught a real pre-existing `LT01` spacing bug from `cdb816d`, fixed alongside). **dbt-as-Dagster-assets uses classic `@dbt_assets`, not `dagster_dbt.DbtProjectComponent`** — the Component reliably hit a Windows `PermissionError` rebuilding its `.local_defs_state` cache on every reload after the first (not transient — reproduced 3+ times; most likely this repo's location inside a continuously-synced folder colliding with the Component's delete-then-recreate caching). A separate, unrelated path-length issue (`LongPathsEnabled=0`) was found and fixed along the way (one-time elevated registry change) but didn't resolve the permission error. `@dbt_assets` reads the manifest directly with no state-staging directory and is stable. Result: 33 dbt-model assets (7 bronze + 12 silver + 7 gold + 7 serving) load with zero errors and **no live Fabric session** (`DbtProject.prepare_if_dev()` only runs `dbt parse`) — materializing them is still blocked pending `az login` (Azure CLI refresh token expired, needs interactive re-auth). Landing assets (Capability 2) and the scheduled delta-load job (Capability 3) not yet built. Full detail: `specs/003-dagster-orchestration/research.md` Decisions 8/10. |
