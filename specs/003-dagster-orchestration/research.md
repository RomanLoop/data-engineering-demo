...
**Outcome (taken, 2026-09-11)**: `DbtProjectComponent` loaded successfully exactly once per manual cache-clear, then reliably failed on every subsequent reload with `PermissionError: [WinError 5] Access denied` while its `StateBackedComponent` base class tried to `rmtree` and rebuild `orchestration/src/orchestration/defs/.local_defs_state/DbtProjectComponent__dbt__/...` before re-copying the compiled dbt artifacts. Reproduced 3+ times (clear → succeed → fail → clear → succeed → fail), so not a transient flake — most likely this repo's location inside a continuously-synced cloud folder ("Meine Ablage") holding a transient lock on just-created files, colliding with the Components system's aggressive delete-then-recreate caching. Along the way, a *separate*, now-fixed issue also surfaced first: Windows long-path support was off (`LongPathsEnabled=0`), which caused a `shutil.Error [WinError 3]` on the even-more-deeply-nested compiled-test paths (e.g. `.../target/compiled/.../gold_customers.yml/unique_gold_customers_customer_key.sql`); the user enabled it (`Set-ItemProperty HKLM:\SYSTEM\CurrentControlSet\Control\FileSystem LongPathsEnabled 1`, elevated, one-time, system-wide), which fixed *that* error but not the subsequent permission one. Switched to `@dbt_assets` (`orchestration/src/orchestration/dbt_assets.py` + a hand-assembled `Definitions` in `definitions.py`) — no nested state-staging directory, reads `manifest.json` directly. Verified stable across 3 consecutive `dg check defs` reloads with zero errors, and `dg list defs` confirms exactly 33 model assets (7 bronze + 12 silver + 7 gold + 7 serving) with dbt tests present as asset checks — matching data-model.md exactly, with zero live Fabric connection.
